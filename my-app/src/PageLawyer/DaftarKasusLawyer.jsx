...
const DaftarKasusLawyer = () => {
    ...
    const fetchSemuaKasus = async () => {
        setLoading(true);
        try {
            const response = await axios.get("https://ppsi-production.up.railway.app/api/kasus");
    ...
    const checkBankAccount = async () => { try { if (!lawyer?.id) return; const res = await axios.get(`https://ppsi-production.up.railway.app/api/pengacara/check-bank/${lawyer.id}`); const { bank_name, account_name, account_number } = res.data; if (!bank_name || !account_name || !account_number) setShowNotification(true); else setShowNotification(false); } catch (error) { console.error("Gagal cek data rekening bank:", error); setShowNotification(true); } };
    const showToast = (message, isError = false) => { setToast({ message, isError }); setTimeout(() => setToast(null), 3000); };
    const handleAmbilKasus = async (kasusId) => { try { const response = await axios.put(`https://ppsi-production.up.railway.app/api/kasus/ambil/${kasusId}`, { lawyer_id: lawyer.id }); showToast(response.data.message); await fetchSemuaKasus(); setTab('Menunggu'); } catch (error) { showToast(error.response?.data?.message || "Terjadi kesalahan.", true); } };
    const handleUpdateStatus = async (id, newStatus, userId) => { try { await axios.put(`https://ppsi-production.up.railway.app/api/kasus/update-status/${id}`, { status: newStatus }); await axios.post("https://ppsi-production.up.railway.app/api/kasus/log-aktivitas", { id_pengguna: userId, aktivitas: `Status kasus ID ${id} diubah menjadi '${newStatus}'` }); showToast("Status berhasil diperbarui"); await fetchSemuaKasus(); } catch (error) { showToast("Gagal memperbarui status kasus.", true); } };
    const fetchLogAktivitas = async (userId) => { try { const response = await axios.get(`https://ppsi-production.up.railway.app/api/kasus/log-aktivitas/${userId}`); setLogAktivitas(response.data); setShowLogModal(true); } catch (error) { console.error("Gagal mengambil log:", error); } };
    ...
    return (
        <div className="DaftarKasusLawyer">
            ...
            {showBuktiModal && ( <div className="modal-overlay" onClick={() => setShowBuktiModal(false)}><div className="modal-content" style={{ width: "90%", maxWidth: "700px" }} onClick={(e) => e.stopPropagation()}><h3 style={{ textAlign: "center" }}>Pratinjau Bukti</h3>{buktiPreview.endsWith(".pdf") ? (<iframe src={`https://ppsi-production.up.railway.app/uploads/${buktiPreview}`} title="PDF Preview" width="100%" height="500px"></iframe>) : (<img src={`https://ppsi-production.up.railway.app/uploads/${buktiPreview}`} alt="Bukti" style={{ maxWidth: "100%", maxHeight: "500px", display: "block", margin: "auto" }}/>)}<button className="btn-close-modal" onClick={() => setShowBuktiModal(false)}>Tutup</button></div></div> )}
