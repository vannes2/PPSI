...
const DaftarKasusLawyer = () => {
  ...
  useEffect(() => {
    if (!lawyer || lawyer.role !== "pengacara") {
      setToast({
        message: "Data pengacara tidak ditemukan. Harap login ulang.",
        isError: true,
      });
      return;
    }
  ...
  const handleAmbilKasus = async (kasusId) => {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/kasus/ambil/${kasusId}`,
        { lawyer_id: lawyer.id }
      );
  ...
  const handleUpdateStatus = async (id, newStatus, userId) => {
    try {
      await axios.put(`http://localhost:5000/api/kasus/update-status/${id}`, {
        status: newStatus,
      });
  ...
  const handleExportPDF = () => {
    ...
    const data = filteredKasus.map((k, i) => [
      ...
      k.area_praktik,
      k.jenis_pengerjaan,
      `Rp${Number(k.biaya_min).toLocaleString()} - Rp${Number(
        k.biaya_max
      ).toLocaleString()}`,
  ...
  const fetchLogAktivitas = async (userId) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/kasus/log-aktivitas/${userId}`
      );
    ...
    <div className="DaftarKasusLawyer">
      ...
      <div className="container-kasus">
        ...
        <div className="tab-kasus">
          ...
          <button className="btn-update" onClick={handleExportPDF}>
            Export PDF
          </button>
        </div>
            ...
            <tbody>
              {filteredKasus.map((kasus) => (
                <tr key={kasus.id}>
                  ...
                  <td>
                    Rp{Number(kasus.biaya_min).toLocaleString()} - Rp
                    {Number(kasus.biaya_max).toLocaleString()}
                  </td>
                  ...
                  <td>
                    <div className="btn-group">
                      <button
                        ...
                        onClick={() => handleNot(kasus.deskripsi)}
                      >
                        Not
                      </button>
                      {tab !== "Belum Diambil" ? (
                        <>
                          ...
                          {kasus.bukti && (
                            <button
                              className="btn-riwayat"
                              onClick={() => handlePreviewBukti(kasus.bukti)}
                            >
                              Bukti
                            </button>
                          )}
                        ...
                        <button
                          className="btn-update"
                          onClick={() => handleAmbilKasus(kasus.id)}
                        >
                          Ambil Kasus
                        </button>
                      )}
